...
.PHONY: all clean test-p1 valgrind-p1 test-p2 valgrind-p2 \
...
# src/store.c is required here now that server.c's dispatch (Phase 2
# Stage 3) calls kv_store_*() unconditionally -- server.o needs store.o
# at link time regardless of which stage's test is linking it in.
$(BUILD)/p1_stage04: tests/p1_stage04_server_loop.c src/net.c src/protocol.c src/command.c src/server.c src/store.c tests/helpers/test_client.c | $(BUILD)
	$(CC) $(CFLAGS) -o $@ $^ $(LDFLAGS)

$(BUILD)/p1_stage05: tests/p1_stage05_server_lifecycle.c src/net.c src/protocol.c src/command.c src/server.c src/store.c tests/helpers/test_client.c | $(BUILD)
	$(CC) $(CFLAGS) -o $@ $^ $(LDFLAGS)
...
# --- Phase 2 ---------------------------------------------------------------
# See docs/stages/phase2-storage.md. Stages 1-2 are pure store logic;
# Stage 3 edits src/server.c's dispatch (see the NOTE above the Phase 1
# stage 4/5 rules -- once you've done that edit, add src/store.c to
# those two rules' prerequisites as well, or they'll fail to link).

$(BUILD)/p2_stage01: tests/p2_stage01_store_set_get.c src/store.c | $(BUILD)
	$(CC) $(CFLAGS) -o $@ $^ $(LDFLAGS)

$(BUILD)/p2_stage02: tests/p2_stage02_store_delete_destroy.c src/store.c | $(BUILD)
	$(CC) $(CFLAGS) -o $@ $^ $(LDFLAGS)

$(BUILD)/p2_stage03: tests/p2_stage03_store_over_wire.c src/net.c src/protocol.c src/command.c src/server.c src/store.c tests/helpers/test_client.c | $(BUILD)
	$(CC) $(CFLAGS) -o $@ $^ $(LDFLAGS)
...
# ---------------------------------------------------------------------------
# Later phases (Phase 3 onward) add their own src/test groups and
